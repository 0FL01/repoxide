...
import { RepoxideError } from '../../../src/shared/errorHandle.js';
...
describe('fileSearch', () => {
  ...
  describe('getIgnoreFilePaths', () => {
    test('should return correct paths when .ignore and .repoxideignore are enabled (.gitignore handled by gitignore option)', async () => {
      ...
      expect(filePatterns).toEqual(['**/.ignore', '**/.repoxideignore']);
    });
    ...
    test('should not include .ignore when useDotIgnore is false', async () => {
      ...
      const filePatterns = await getIgnoreFilePatterns(mockConfig);
      // .gitignore is not included because it's handled by globby's gitignore option
      expect(filePatterns).toEqual(['**/.repoxideignore']);
    });
  ...
  describe('getIgnorePatterns', () => {
    ...
    test('should include custom patterns', async () => {
      ...
      expect(patterns).toEqual(['repoxide-output.xml', '*.custom', 'temp/']);
    });
...
.DS_Store
  ...
  describe('filterFiles', () => {
    ...
    test('should call globby with correct parameters', async () => {
      ...
      expect(globby).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          gitignore: true,
          ignoreFiles: expect.arrayContaining(['**/.repoxideignore']),
          onlyFiles: true,
          absolute: false,
          dot: true,
    ...
    test.runIf(!isWindows)('should respect parent directory .gitignore patterns (v16 behavior)', async () => {
      ...
      // matching Git's standard behavior. This makes Repoxide's file filtering align with Git's expectations.
      const mockConfig = createMockConfig({
        include: ['**/*.js'],
        ignore: {
          useGitignore: true,
    ...
    test.runIf(!isWindows)('should respect parent directory .repoxideignore patterns', async () => {
      // This test verifies that .repoxideignore files in parent directories are respected.
      // .repoxideignore is always enabled by default.
      const mockConfig = createMockConfig({
        include: ['**/*.js'],
        ignore: {
          useGitignore: false,
      ...
      // Simulate parent .repoxideignore pattern applying to subdirectory files
      const mockFileStructure = [
        'root/file1.js',
        'root/subdir/file2.js',
        'root/subdir/nested/file3.js',
        // 'root/subdir/nested/ignored-by-repoxide.js' - filtered by parent .repoxideignore
      ];

      const mockIgnoreContent = {
        '/mock/root/.repoxideignore': 'ignored-by-repoxide.js',
      };

      vi.mocked(globby).mockImplementation(async () => {
        // Simulate parent .repoxideignore patterns applying to all subdirectories
        return mockFileStructure.filter((file) => {
          const basename = path.basename(file);
          const parentIgnore = mockIgnoreContent['/mock/root/.repoxideignore'];
          if (minimatch(basename, parentIgnore)) {
            return false;
          }
          return true;
      ...
      // Verify parent .repoxideignore pattern filtered out the file
      expect(result.filePaths).toHaveLength(3);
      expect(result.filePaths).toContain('root/file1.js');
      expect(result.filePaths).toContain('root/subdir/file2.js');
      expect(result.filePaths).toContain('root/subdir/nested/file3.js');
      expect(result.filePaths).not.toContain('root/subdir/nested/ignored-by-repoxide.js');
      expect(result.emptyDirPaths).toEqual([]);

      // Verify ignoreFiles option includes .repoxideignore
      expect(globby).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({
          ignoreFiles: expect.arrayContaining(['**/.repoxideignore']),
  ...
  describe('searchFiles path validation', () => {
    test('should throw error when target path does not exist', async () => {
      ...
      await expect(searchFiles('/nonexistent/path', mockConfig)).rejects.toThrow(RepoxideError);
    ...
    test('should throw error when target path is a file, not a directory', async () => {
      ...
      await expect(searchFiles('/path/to/file.txt', mockConfig)).rejects.toThrow(RepoxideError);
