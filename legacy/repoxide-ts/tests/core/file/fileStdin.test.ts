...
describe('fileStdin', () => {
  ...
  describe('readFilePathsFromStdin', () => {
    ...
    it('should throw error when stdin is TTY', async () => {
      ...
      await expect(readFilePathsFromStdin(cwd, deps)).rejects.toThrow(RepoxideError);
      await expect(readFilePathsFromStdin(cwd, deps)).rejects.toThrow(
        'No data provided via stdin. Please pipe file paths to repoxide when using --stdin flag.',
      );
    });
    ...
    it('should wrap generic errors in RepoxideError', async () => {
      const mockInterface = {
    ...
    it('should propagate RepoxideError without wrapping', async () => {
      const mockStdin = { isTTY: true } as NodeJS.ReadableStream & { isTTY?: boolean };
      const deps: StdinDependencies = {
        stdin: mockStdin,
        createReadlineInterface: vi.fn(),
      ...
      expect(error).toBeInstanceOf(RepoxideError);
