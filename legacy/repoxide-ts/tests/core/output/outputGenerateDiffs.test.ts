...
import type { RepoxideConfigMerged } from '../../../src/config/configSchema.js';
...
describe('Output Generation with Diffs', () => {
  ...
  const mockConfig: RepoxideConfigMerged = createMockConfig({
    cwd: '/test',
    output: {
      files: true,
  ...
  test('XML style output with parsableStyle should include diffs section', async () => {
    ...
    mockDeps.generateParsableXmlOutput.mockImplementation((renderContext: RenderContext) => {
      ...
      return `<repoxide><diffs>${renderContext.gitDiffWorkTree}</diffs></repoxide>`;
    });
    ...
    // Verify the diffs are included in the output
    expect(output).toContain('<repoxide><diffs>');
    expect(output).toContain(sampleDiff);
    expect(output).toContain('</diffs></repoxide>');
