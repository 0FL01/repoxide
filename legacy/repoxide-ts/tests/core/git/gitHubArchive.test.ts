  ...
  describe('downloadGitHubArchive', () => {
    ...
    test('should successfully download and extract archive', async () => {
      ...
      mockUnzip.mockImplementation((_data, callback) => {
        const extracted = {
          'repoxide-main/': new Uint8Array(0), // Directory
          'repoxide-main/test.txt': new Uint8Array([0x68, 0x65, 0x6c, 0x6c, 0x6f]), // "hello"
        };
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://github.com/yamadashy/repoxide/archive/refs/heads/main.zip',
      ...
      // Verify cleanup
      expect(mockFs.unlink).toHaveBeenCalledWith(path.join(mockTargetDirectory, 'repoxide-main.zip'));
    });
    ...
    test('should try fallback URLs on 404', async () => {
      ...
      const repoInfoNoRef = { owner: 'yamadashy', repo: 'repoxide' };
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://github.com/yamadashy/repoxide/archive/HEAD.zip',
        expect.any(Object),
      );
      expect(mockFetch).toHaveBeenCalledWith(
        'https://github.com/yamadashy/repoxide/archive/refs/heads/master.zip',
        expect.any(Object),
      );
    });
    ...
    test('should handle path traversal attack', async () => {
      ...
      mockUnzip.mockImplementation((_data, callback) => {
        const extracted = {
          'repoxide-main/../../../etc/passwd': new Uint8Array([0x65, 0x76, 0x69, 0x6c]), // "evil"
          'repoxide-main/safe.txt': new Uint8Array([0x73, 0x61, 0x66, 0x65]), // "safe"
        };
        callback(null, extracted);
      });
