...
import type { RepoxideConfigFile, RepoxideConfigMerged, RepoxideOutputStyle } from '../../src/config/configSchema.js';
...
describe.runIf(!isWindows)('packager integration', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'repoxide-test-'));
  });
  ...
  for (const { desc, input, output, config } of testCases) {
    test(`should correctly pack ${desc}`, async () => {
      ...
      const fileConfig: RepoxideConfigFile = await loadFileConfig(inputDir, null);
      const mergedConfig: RepoxideConfigMerged = mergeConfigs(process.cwd(), fileConfig, {
        output: {
          filePath: actualOutputPath,
          style: (config.output?.style || 'plain') as RepoxideOutputStyle,
