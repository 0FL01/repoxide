...
describe('getGlobalDirectory', () => {
  ...
  describe('Windows platform', () => {
    test('should use LOCALAPPDATA when available', () => {
      ...
      expect(result).toBe(path.join('C:\\Users\\Test\\AppData\\Local', 'Repoxide'));
    });
  ...
  describe('Unix platforms', () => {
    test('should use XDG_CONFIG_HOME when available', () => {
      ...
      const result = getGlobalDirectory();
      expect(result).toBe(path.join('/custom/config', 'repoxide'));
    });
    ...
    test('should fall back to ~/.config on Linux', () => {
      ...
      const result = getGlobalDirectory();
      expect(result).toBe(path.join('/home/test', '.config', 'repoxide'));
    });
    ...
    test('should fall back to ~/.config on macOS', () => {
      ...
      const result = getGlobalDirectory();
      expect(result).toBe(path.join('/Users/test', '.config', 'repoxide'));
    });
  });
  ...
  describe('Edge cases', () => {
    test('should handle empty homedir', () => {
      ...
      const result = getGlobalDirectory();
      expect(result).toBe(path.join('', '.config', 'repoxide'));
    });
    ...
    test('should handle unusual XDG_CONFIG_HOME paths', () => {
      ...
      const result = getGlobalDirectory();
      expect(result).toBe(path.join('////multiple///slashes///', 'repoxide'));
    });
  });
});
