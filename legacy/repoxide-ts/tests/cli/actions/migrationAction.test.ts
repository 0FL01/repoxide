  ...
  test('should migrate all files when they exist', async () => {
    ...
    expect(fs.writeFile).toHaveBeenCalledWith(
      ...
      JSON.stringify(
        {
          output: {
            filePath: 'repoxide-output.txt',
            instructionFilePath: 'repoxide-instruction.md',
          },
        },
        null,
        2,
    ...
    expect(fs.writeFile).toHaveBeenCalledWith(newIgnorePath, 'repoxide-output.txt\n*.log', 'utf8');
    expect(fs.writeFile).toHaveBeenCalledWith(newInstructionPath, '# Repoxide Instructions', 'utf8');
