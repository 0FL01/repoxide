...
describe('initAction', () => {
  ...
  describe('createConfigFile', () => {
    it('should create a new local config file when one does not exist', async () => {
      ...
      const configPath = path.resolve('/test/dir/repoxide.config.json');
    ...
    it('should create a new global config file when one does not exist', async () => {
      ...
      vi.mocked(prompts.confirm).mockResolvedValue(true);
      vi.mocked(getGlobalDirectory).mockImplementation(() => '/global/repoxide');

      await createConfigFile('/test/dir', true);

      const configPath = path.resolve('/global/repoxide/repoxide.config.json');
  ...
  describe('createIgnoreFile', () => {
    it('should not create a new .repoxideignore file when global flag is set', async () => {
    ...
    it('should create a new .repoxideignore file when one does not exist', async () => {
      vi.mocked(fs.access).mockRejectedValue(new Error('File does not exist'));
      vi.mocked(prompts.confirm).mockResolvedValue(true);

      await createIgnoreFile('/test/dir', false);

      const ignorePath = path.resolve('/test/dir/.repoxideignore');
    ...
    it('should prompt to overwrite when .repoxideignore file already exists', async () => {
      vi.mocked(fs.access).mockResolvedValue(undefined);
    ...
    it('should return false when user chooses not to create .repoxideignore', async () => {
      vi.mocked(prompts.confirm).mockResolvedValue(false);
