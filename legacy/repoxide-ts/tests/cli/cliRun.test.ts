...
import { logger, type RepoxideLogLevel, repoxideLogLevels } from '../../src/shared/logger.js';
import { createMockConfig } from '../testing/testUtils.js';

let logLevel: RepoxideLogLevel;
...
vi.mock('../../src/shared/logger', () => ({
  ...
  logger: {
    ...
    success: vi.fn(),
    note: vi.fn(),
    setLogLevel: vi.fn((level: RepoxideLogLevel) => {
      logLevel = level;
    }),
...
describe('cliRun', () => {
  ...
  describe('executeAction', () => {
    ...
    test('should enable verbose logging when verbose option is true', async () => {
      ...
      expect(logger.setLogLevel).toHaveBeenCalledWith(repoxideLogLevels.DEBUG);
    });
    ...
    test('should execute remote action when remote option is provided', async () => {
      ...
      expect(remoteAction.runRemoteAction).toHaveBeenCalledWith('yamadashy/repoxide', expect.any(Object));
      expect(defaultAction.runDefaultAction).not.toHaveBeenCalled();
    });
  });
  ...
  describe('quiet mode', () => {
    test('should set log level to SILENT when quiet option is true', async () => {
      ...
      expect(logger.getLogLevel()).toBe(repoxideLogLevels.SILENT);
    });
    ...
    test('should set log level to DEBUG when verbose option is true', async () => {
      ...
      expect(logger.getLogLevel()).toBe(repoxideLogLevels.DEBUG);
    });
    ...
    test('should set log level to INFO by default', async () => {
      ...
      expect(logger.getLogLevel()).toBe(repoxideLogLevels.INFO);
    });
  });
