...
describe('mcpToolRuntime', () => {
  ...
  describe('createToolWorkspace', () => {
    beforeEach(() => {
      ...
      vi.mocked(fs.mkdtemp).mockResolvedValue('/tmp/repoxide/mcp-outputs/temp-dir');
    });
    ...
    it('should create a temporary directory for tool operations', async () => {
      ...
      expect(path.join).toHaveBeenCalledWith('/tmp', 'repoxide', 'mcp-outputs');
      expect(fs.mkdir).toHaveBeenCalledWith('/tmp/repoxide/mcp-outputs', { recursive: true });
      expect(fs.mkdtemp).toHaveBeenCalledWith('/tmp/repoxide/mcp-outputs/');
      expect(tempDir).toBe('/tmp/repoxide/mcp-outputs/temp-dir');
    });
