...
export const analyzeContent = (config: RepoxideConfigMerged): ContentInfo => {
  return {
    selection: {
...
export const generateHeader = (config: RepoxideConfigMerged, _generationDate: string): string => {
  ...
  return `${description}, combined into a single document by Repoxide.\n${processingInfo}`.trim();
};

export const generateSummaryPurpose = (config: RepoxideConfigMerged): string => {
  const info = analyzeContent(config);
...
export const generateSummaryUsageGuidelines = (config: RepoxideConfigMerged, repositoryInstruction: string): string => {
  return `
...
export const generateSummaryNotes = (config: RepoxideConfigMerged): string => {
  const info = analyzeContent(config);
  const notes = [
    "- Some files may have been excluded based on .gitignore rules and Repoxide's configuration",
