...
import type { RepoxideProgressCallback } from '../../shared/types.js';
...
const makeChunkConfig = (baseConfig: RepoxideConfigMerged, partIndex: number): RepoxideConfigMerged => {
  if (partIndex === 1) {
    return baseConfig;
  }
...
const renderGroups = async (
  ...
  partIndex: number,
  rootDirs: string[],
  baseConfig: RepoxideConfigMerged,
...
export const generateSplitOutputParts = async ({
  ...
  gitDiffResult: GitDiffResult | undefined;
  gitLogResult: GitLogResult | undefined;
  progressCallback: RepoxideProgressCallback;
  deps: {
    generateOutput: GenerateOutputFn;
  };
}): Promise<OutputSplitPart[]> => {
  if (!Number.isSafeInteger(maxBytesPerPart) || maxBytesPerPart <= 0) {
    throw new RepoxideError(`Invalid maxBytesPerPart: ${maxBytesPerPart}`);
  }
