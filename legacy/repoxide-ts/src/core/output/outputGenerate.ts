...
const getCompiledTemplate = (style: string): Handlebars.TemplateDelegate => {
  ...
  switch (style) {
    ...
    case 'plain':
      ...
      break;
    default:
      throw new RepoxideError(`Unsupported output style for handlebars template: ${style}`);
  }
...
export const generateOutput = async (
  ...
  switch (config.output.style) {
    ...
    default:
      throw new RepoxideError(`Unsupported output style: ${config.output.style}`);
  }
};
...
export const buildOutputGeneratorContext = async (
  ...
  if (config.output.instructionFilePath) {
    ...
    try {
      ...
      throw new RepoxideError(`Instruction file not found at ${instructionPath}`);
    }
  }
