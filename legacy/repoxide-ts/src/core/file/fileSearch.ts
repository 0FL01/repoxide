...
import type { RepoxideConfigMerged } from '../../config/configSchema.js';
import { defaultIgnoreList } from '../../config/defaultIgnore.js';
import { RepoxideError } from '../../shared/errorHandle.js';
...
export const searchFiles = async (
  ...
  try {
    ...
    if (error instanceof Error && 'code' in error) {
      ...
      if (errorCode === 'ENOENT') {
        throw new RepoxideError(`Target path does not exist: ${rootDir}`);
      }
      ...
      throw new RepoxideError(`Failed to access path: ${rootDir}. Error code: ${errorCode}. ${error.message}`);
    }
    // Preserve original error stack trace for debugging
    const repoxideError = new RepoxideError(
      `Failed to access path: ${rootDir}. Reason: ${error instanceof Error ? error.message : JSON.stringify(error)}`,
    );
    repoxideError.cause = error;
    throw repoxideError;
  }

  // Check if the path is a directory
  if (!pathStats.isDirectory()) {
    throw new RepoxideError(
...
export const getIgnoreFilePatterns = async (config: RepoxideConfigMerged): Promise<string[]> => {
  ...
  //
  // Multiple ignore files in the same directory (.gitignore, .ignore, .repoxideignore)
  ...
  ignoreFilePatterns.push('**/.repoxideignore');

  return ignoreFilePatterns;
};

export const getIgnorePatterns = async (rootDir: string, config: RepoxideConfigMerged): Promise<string[]> => {
 ...
 */
export const listDirectories = async (rootDir: string, config: RepoxideConfigMerged): Promise<string[]> => {
  const { adjustedIgnorePatterns, ignoreFilePatterns } = await prepareIgnoreContext(rootDir, config);
 ...
 */
export const listFiles = async (rootDir: string, config: RepoxideConfigMerged): Promise<string[]> => {
  const { adjustedIgnorePatterns, ignoreFilePatterns } = await prepareIgnoreContext(rootDir, config);
