...
export const readFilePathsFromStdin = async (
  ...
  try {
    ...
    if (stdin.isTTY) {
      throw new RepoxideError('No data provided via stdin. Please pipe file paths to repoxide when using --stdin flag.');
    }
    ...
    if (validLines.length === 0) {
      throw new RepoxideError('No valid file paths found in stdin input.');
    }
    ...
    if (error instanceof RepoxideError) {
      throw error;
    }

    if (error instanceof Error) {
      throw new RepoxideError(`Failed to read file paths from stdin: ${error.message}`);
    }

    throw new RepoxideError('An unexpected error occurred while reading from stdin.');
  }
};
