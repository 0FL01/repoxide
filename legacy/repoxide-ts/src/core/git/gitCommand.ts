 ...
 * @throws {RepoxideError} If the URL is invalid or contains potentially dangerous parameters
 */
export const validateGitUrl = (url: string): void => {
  if (url.includes('--upload-pack') || url.includes('--config') || url.includes('--exec')) {
    throw new RepoxideError(`Invalid repository URL. URL contains potentially dangerous parameters: ${url}`);
  }

  // Check if the URL starts with git@ or https://
  if (!(url.startsWith('git@') || url.startsWith('https://'))) {
    throw new RepoxideError(`Invalid URL protocol for '${url}'. URL must start with 'git@' or 'https://'`);
  }
  ...
  try {
    ...
    throw new RepoxideError(`Invalid repository URL. Please provide a valid URL: ${redactedUrl}`);
  }
};
