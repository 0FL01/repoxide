    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        const isNotFoundError =
          lastError instanceof RepoxideError &&
...
const downloadFile = async (
  ...
  try {
    ...
    if (!response.body) {
      throw new RepoxideError('No response body received');
    }
...
const extractZipArchive = async (
  ...
  try {
    ...
    throw new RepoxideError(`Failed to extract archive: ${(error as Error).message}`);
  }
};
...
const extractZipArchiveInMemory = async (
  ...
  await new Promise<void>((resolve, reject) => {
    unzip(zipUint8Array, (err, extracted) => {
      if (err) {
        reject(new RepoxideError(`Failed to extract ZIP archive: ${err.message}`));
        return;
      }
