...
export class LanguageParser {
  ...
  private async prepareLang(name: SupportedLang): Promise<LanguageResources> {
    try {
      ...
      if (!config) {
        throw new RepoxideError(`Language configuration not found for: ${name}`);
      }
      ...
      throw new RepoxideError(`Failed to prepare language ${name}: ${message}`);
    }
  }

  private async getResources(name: SupportedLang): Promise<LanguageResources> {
    if (!this.initialized) {
      throw new RepoxideError('LanguageParser is not initialized. Call init() first.');
    }
  ...
  public async init(): Promise<void> {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new RepoxideError(`Failed to initialize parser: ${message}`);
    }
  }
