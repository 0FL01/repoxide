...
import { handleError, RepoxideError } from '../shared/errorHandle.js';
import { logger, repoxideLogLevels } from '../shared/logger.js';
...
export const run = async () => {
  try {
    program
      .description('Repoxide - Pack your repository into a single AI-friendly file')
      ...
      .option(
        ...
        (value: string | boolean) => {
          if (typeof value === 'string') {
            if (!/^\d+$/.test(value)) {
              throw new RepoxideError(`Invalid token count threshold: '${value}'. Must be a non-negative integer.`);
            }
            return Number(value);
          }
          return value;
      ...
      .option(
        ...
        (v: string) => {
          if (!/^\d+$/.test(v)) {
            throw new RepoxideError(`Invalid number for --top-files-len: '${v}'. Must be a non-negative integer.`);
          }
          return Number(v);
        },
      )
      // Repoxide Output Options
      .optionsGroup('Repoxide Output Options')
      .option('-o, --output <file>', 'Output file path (default: repoxide-output.xml, use "-" for stdout)')
      ...
      .addOption(
        new Option(
          '--split-output <size>',
          'Split output into multiple numbered files (e.g., repoxide-output.1.xml, repoxide-output.2.xml); size like 500kb, 2mb, or 2.5mb',
      ...
      .option(
        ...
        (v: string) => {
          if (!/^\d+$/.test(v)) {
            throw new RepoxideError(`Invalid number for --include-logs-count: '${v}'. Must be a non-negative integer.`);
          }
          return Number(v);
        },
      )
      ...
      .option('-c, --config <path>', 'Use custom config file instead of repoxide.config.json')
      .option('--init', 'Create a new repoxide.config.json file with defaults')
...
export const runCli = async (directories: string[], cwd: string, options: CliOptions) => {
  ...
  if (options.quiet) {
    logger.setLogLevel(repoxideLogLevels.SILENT);
  } else if (options.verbose) {
    logger.setLogLevel(repoxideLogLevels.DEBUG);
  } else {
    logger.setLogLevel(repoxideLogLevels.INFO);
  }
