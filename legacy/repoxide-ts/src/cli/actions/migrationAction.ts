...
/**
 * Replace all occurrences of 'repopack' with 'repoxide' in a string
 */
const replaceRepopackString = (content: string): string => {
  return content.replace(/repopack/g, 'repoxide').replace(/Repopack/g, 'Repoxide');
};

/**
 * Update file content by replacing 'repopack' with 'repoxide'
 */
...
const updateInstructionPath = (content: string): string => {
  try {
    ...
    if (config.output?.instructionFilePath) {
      config.output.instructionFilePath = config.output.instructionFilePath.replace('repopack', 'repoxide');
    }
    // Also update output.filePath if it exists
    if (config.output?.filePath) {
      config.output.filePath = config.output.filePath.replace('repopack', 'repoxide');
    }
...
const getOutputFilePaths = (rootDir: string): { oldPaths: string[]; newPaths: string[] } => {
  ...
  const newPaths = extensions.map((ext) => path.join(rootDir, `repoxide-output${ext}`));
...
/**
 * Update content of gitignore and repoxideignore files
 */
const updateIgnoreFiles = async (rootDir: string): Promise<void> => {
  const gitignorePath = path.join(rootDir, '.gitignore');
  const repoxideignorePath = path.join(rootDir, '.repoxideignore');
  ...
  if (await fileExists(repoxideignorePath)) {
    const updated = await updateFileContent(repoxideignorePath);
    if (!updated) {
      logger.debug('No changes needed in .repoxideignore');
    }
  }
};
...
const getMigrationPaths = (rootDir: string): MigrationPaths => {
  ...
  return {
    ...
    newConfigPath: path.join(rootDir, 'repoxide.config.json'),
    oldIgnorePath: path.join(rootDir, '.repopackignore'),
    newIgnorePath: path.join(rootDir, '.repoxideignore'),
    oldInstructionPath: path.join(rootDir, 'repopack-instruction.md'),
    newInstructionPath: path.join(rootDir, 'repoxide-instruction.md'),
    oldOutputPaths,
    newOutputPaths,
    oldGlobalConfigPath: path.join(oldGlobalDirectory, 'repopack.config.json'),
    newGlobalConfigPath: path.join(newGlobalDirectory, 'repoxide.config.json'),
  };
};
...
export const runMigrationAction = async (rootDir: string): Promise<MigrationResult> => {
  ...
  try {
    ...
    migrationMessage += `${items.join(' and ')}. Would you like to migrate to ${pc.green('Repoxide')}?`;
    ...
    logger.info(pc.cyan('\nMigrating from Repopack to Repoxide...'));
    logger.log('');
    ...
    // Update content in gitignore and repoxideignore
    ...
    if (
      ...
      logger.log('');
      logger.info(
        'You can now use Repoxide commands as usual. The old Repopack files have been migrated to the new format.',
      );
      logger.log('');
    }
