...
export const runDefaultAction = async (
  ...
  const fileConfig: RepoxideConfigFile = await loadFileConfig(cwd, cliOptions.config ?? null);
  logger.trace('Loaded file config:', fileConfig);

  // Parse the CLI options into a config
  const cliConfig: RepoxideConfigCli = buildCliConfig(cliOptions);
  logger.trace('CLI config:', cliConfig);

  // Merge default, file, and CLI configs
  const config: RepoxideConfigMerged = mergeConfigs(cwd, fileConfig, cliConfig);
 ...
 */
export const buildCliConfig = (options: CliOptions): RepoxideConfigCli => {
  ...
  if (options.style) {
    cliConfig.output = {
      ...cliConfig.output,
      style: options.style.toLowerCase() as RepoxideOutputStyle,
    };
  }
  ...
  try {
    return repoxideConfigCliSchema.parse(cliConfig);
  } catch (error) {
 ...
 * Throws RepoxideError if incompatible options are detected.
 */
const validateConflictingOptions = (config: RepoxideConfigMerged): void => {
  ...
  for (const [optionA, optionB, message] of conflicts) {
    if (options[optionA].enabled && options[optionB].enabled) {
      throw new RepoxideError(`${options[optionA].name} cannot be used with ${options[optionB].name}. ${message}`);
    }
  }
};
