...
const performGitClone = async (
  ...
  if (!(await deps.isGitInstalled())) {
    throw new RepoxideError('Git is not installed or not in the system PATH.');
  }
  // Get remote refs
  let refs: string[] = [];
  try {
...
export const cloneRepository = async (
  ...
  try {
    ...
    throw new RepoxideError(`Failed to clone repository: ${(error as Error).message}`);
  }
};
...
Please try running from a different directory (e.g., your home directory or Documents folder).`,
    ...
    throw new RepoxideError(`Failed to copy skill output: ${(error as Error).message}`);
  }
};
...
Please try one of the following:
  ...
  • Use the --output flag to specify a writable location: --output ~/repoxide-output.xml
  • Use --stdout to print output directly to the console`,
      );
    }

    throw new RepoxideError(`Failed to copy output file: ${(error as Error).message}`);
  }
};
