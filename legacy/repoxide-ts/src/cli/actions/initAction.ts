...
import {
  defaultConfig,
  defaultFilePathMap,
  type RepoxideConfigFile,
  type RepoxideOutputStyle,
...
export const runInitAction = async (rootDir: string, isGlobal: boolean): Promise<void> => {
  prompts.intro(pc.bold(`Welcome to Repoxide ${isGlobal ? 'Global ' : ''}Configuration!`));

  try {
    // Step 1: Ask if user wants to create a config file
    const isCreatedConfig = await createConfigFile(rootDir, isGlobal);

    // Step 2: Ask if user wants to create a .repoxideignore file
    ...
    if (!isCreatedConfig && !isCreatedIgnoreFile) {
      ...
      prompts.outro(pc.green('Initialization complete! You can now use Repoxide with your specified settings.'));
    }
  } catch (error) {
...
export const createConfigFile = async (rootDir: string, isGlobal: boolean): Promise<boolean> => {
  const configPath = path.resolve(isGlobal ? getGlobalDirectory() : rootDir, 'repoxide.config.json');

  const isCreateConfig = await prompts.confirm({
    message: `Do you want to create a ${isGlobal ? 'global ' : ''}${pc.green('repoxide.config.json')} file?`,
  });
  if (!isCreateConfig) {
    prompts.log.info(`Skipping ${pc.green('repoxide.config.json')} file creation.`);
    return false;
  }
  ...
  if (isConfigFileExists) {
    const isOverwrite = await prompts.confirm({
      message: `A ${isGlobal ? 'global ' : ''}${pc.green('repoxide.config.json')} file already exists. Do you want to overwrite it?`,
    });
    if (!isOverwrite) {
      prompts.log.info(`Skipping ${pc.green('repoxide.config.json')} file creation.`);
      return false;
  ...
  const options = await prompts.group(
    {
      ...
      outputFilePath: ({ results }) => {
        const defaultFilePath = defaultFilePathMap[results.outputStyle as RepoxideOutputStyle];
        return prompts.text({
  ...
  const config: RepoxideConfigFile = {
    $schema: 'https://repoxide.com/schemas/latest/schema.json',
    ...defaultConfig,
    output: {
      ...defaultConfig.output,
      filePath: options.outputFilePath as string,
      style: options.outputStyle as RepoxideOutputStyle,
    },
  };
...
export const createIgnoreFile = async (rootDir: string, isGlobal: boolean): Promise<boolean> => {
  if (isGlobal) {
    prompts.log.info(`Skipping ${pc.green('.repoxideignore')} file creation for global configuration.`);
    return false;
  }

  const ignorePath = path.resolve(rootDir, '.repoxideignore');
  const createIgnore = await prompts.confirm({
    message: `Do you want to create a ${pc.green('.repoxideignore')} file?`,
  });
  if (!createIgnore) {
    prompts.log.info(`Skipping ${pc.green('.repoxideignore')} file creation.`);
    return false;
  }
  ...
  if (isIgnoreFileExists) {
    const overwrite = await prompts.confirm({
      message: `A ${pc.green('.repoxideignore')} file already exists. Do you want to overwrite it?`,
    });

    if (!overwrite) {
      prompts.log.info(`${pc.green('.repoxideignore')} file creation skipped. Existing file will not be modified.`);
      return false;
    }
  }
