...
import { registerGrepRepoxideOutputTool } from './tools/grepRepoxideOutputTool.js';
...
import { registerReadRepoxideOutputTool } from './tools/readRepoxideOutputTool.js';

/**
 * Instructions for the Repoxide MCP Server that describe its capabilities and usage
 */
const MCP_SERVER_INSTRUCTIONS =
  'Repoxide MCP Server provides AI-optimized codebase analysis tools. ' +
  ...
  'then read_repoxide_output and grep_repoxide_output to analyze it. ' +
...
export const createMcpServer = async () => {
  ...
  registerReadRepoxideOutputTool(mcpServer);
  registerGrepRepoxideOutputTool(mcpServer);
...
export const runMcpServer = async (deps: Dependencies = defaultDependencies) => {
  ...
  const handleExit = async () => {
    try {
      await server.close();
      logger.trace('Repoxide MCP Server shutdown complete');
      processExit(0);
    } catch (error) {
  ...
  try {
    ...
    logger.trace('Repoxide MCP Server running on stdio');
  } catch (error) {
