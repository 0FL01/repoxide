...
const readRepoxideOutputInputSchema = z.object({
  outputId: z.string().describe('ID of the Repoxide output file to read'),
  startLine: z.coerce
    .number()
    .optional()
...
const readRepoxideOutputOutputSchema = z.object({
...
/**
 * Register the tool to read Repoxide output files
 */
export const registerReadRepoxideOutputTool = (mcpServer: McpServer) => {
  mcpServer.registerTool(
    'read_repoxide_output',
    {
      title: 'Read Repoxide Output',
      description:
        'Read the contents of a Repoxide-generated output file. Supports partial reading with line range specification for large files. This tool is designed for environments where direct file system access is limited (e.g., web-based environments, sandboxed applications). For direct file system access, use standard file operations.',
      inputSchema: readRepoxideOutputInputSchema,
      outputSchema: readRepoxideOutputOutputSchema,
      annotations: {
        readOnlyHint: true,
    ...
    async ({ outputId, startLine, endLine }): Promise<CallToolResult> => {
      try {
        logger.trace(`Reading Repoxide output with ID: ${outputId}`);
        ...
        } satisfies z.infer<typeof readRepoxideOutputOutputSchema>);
      } catch (error) {
        logger.error(`Error reading Repoxide output: ${error}`);
