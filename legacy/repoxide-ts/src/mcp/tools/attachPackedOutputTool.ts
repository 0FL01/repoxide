...
const attachPackedOutputInputSchema = z.object({
  path: z
    .string()
    .describe(
      'Path to a directory containing repoxide output file or direct path to a packed repository file (supports .xml, .md, .txt, .json formats)',
    ),
  topFilesLength: z
    .number()
    .int()
...
/**
 * Resolves the path to a repoxide output file and detects its format
 * @param inputPath Path to a directory containing repoxide output file or direct path to a packed repository file
...
async function resolveOutputFilePath(inputPath: string): Promise<{ filePath: string; format: string }> {
  try {
    ...
    if (stats.isDirectory()) {
      // If it's a directory, look for repoxide output files in priority order
      ...
      throw new Error(
        `No repoxide output file found in directory: ${inputPath}. Looking for: ${possibleFiles.join(', ')}`,
      );
    }
...
/**
 * Extract file paths and character counts from a repoxide output XML file
 * @param content The content of the repoxide output XML file
...
export const registerAttachPackedOutputTool = (mcpServer: McpServer) => {
  mcpServer.registerTool(
    'attach_packed_output',
    {
      ...
      description: `Attach an existing Repoxide packed output file for AI analysis.
This tool accepts either a directory containing a repoxide output file or a direct path to a packed repository file.
...
It will return in that case a new output ID and the updated content.`,
    ...
    async ({ path: inputPath, topFilesLength }): Promise<CallToolResult> => {
      try {
        // Resolve the path to the repoxide output file
