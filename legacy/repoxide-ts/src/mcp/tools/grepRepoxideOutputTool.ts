...
const grepRepoxideOutputInputSchema = z.object({
  outputId: z.string().describe('ID of the Repoxide output file to search'),
...
const grepRepoxideOutputOutputSchema = z.object({
...
/**
 * Register the tool to search Repoxide output files with grep-like functionality
 */
export const registerGrepRepoxideOutputTool = (mcpServer: McpServer) => {
  mcpServer.registerTool(
    'grep_repoxide_output',
    {
      title: 'Grep Repoxide Output',
      description:
        'Search for patterns in a Repoxide output file using grep-like functionality with JavaScript RegExp syntax. Returns matching lines with optional context lines around matches.',
      inputSchema: grepRepoxideOutputInputSchema,
      outputSchema: grepRepoxideOutputOutputSchema,
      annotations: {
        readOnlyHint: true,
    ...
    async ({
      ...
      try {
        logger.trace(`Searching Repoxide output with ID: ${outputId}, pattern: ${pattern}`);
        ...
        if (searchResult.matches.length === 0) {
          return buildMcpToolSuccessResponse({
            description: `No matches found for pattern "${pattern}" in Repoxide output file (ID: ${outputId}).`,
            matches: [],
            formattedOutput: [],
            totalMatches: 0,
            pattern,
          } satisfies z.infer<typeof grepRepoxideOutputOutputSchema>);
        }

        return buildMcpToolSuccessResponse({
          description: `Found ${searchResult.matches.length} match(es) for pattern "${pattern}" in Repoxide output file (ID: ${outputId}):`,
        ...
        logger.error(`Error in grep_repoxide_output: ${error}`);
