...
import { RepoxideError, rethrowValidationErrorIfZodError } from '../shared/errorHandle.js';
import { logger } from '../shared/logger.js';
import {
  defaultConfig,
  defaultFilePathMap,
  type RepoxideConfigCli,
  type RepoxideConfigFile,
  type RepoxideConfigMerged,
...
const defaultConfigPaths = [
  ...
  'repoxide.config.mjs',
  'repoxide.config.cjs',
  'repoxide.config.json5',
  'repoxide.config.jsonc',
  'repoxide.config.json',
];
...
export const loadFileConfig = async (
  ...
  if (argConfigPath) {
    ...
    throw new RepoxideError(`Config file not found at ${argConfigPath}`);
  }
  ...
  logger.log(
    pc.dim(
      `No custom config found at ${defaultConfigPaths.join(', ')} or global config at ${globalConfigPaths.join(', ')}.\nYou can add a config file for additional settings. Please check https://github.com/yamadashy/repoxide for more information.`,
    ),
  );
  return {};
};
...
const loadAndValidateConfig = async (
  ...
  try {
    ...
    switch (ext) {
      ...
      default:
        throw new RepoxideError(`Unsupported config file format: ${filePath}`);
    }

    return repoxideConfigFileSchema.parse(config);
  } catch (error) {
    rethrowValidationErrorIfZodError(error, 'Invalid config schema');
    if (error instanceof SyntaxError) {
      throw new RepoxideError(`Invalid syntax in config file ${filePath}: ${error.message}`);
    }
    if (error instanceof Error) {
      throw new RepoxideError(`Error loading config from ${filePath}: ${error.message}`);
    }
    throw new RepoxideError(`Error loading config from ${filePath}`);
  }
};

export const mergeConfigs = (
  cwd: string,
  fileConfig: RepoxideConfigFile,
  cliConfig: RepoxideConfigCli,
): RepoxideConfigMerged => {
  ...
  try {
    return repoxideConfigMergedSchema.parse(mergedConfig);
  } catch (error) {
