import { RepoxideError } from './errorHandle.js';
...
export const parseHumanSizeToBytes = (input: string): number => {
  ...
  if (!Number.isFinite(amount) || amount <= 0) {
    throw new RepoxideError(`Invalid size amount: '${match[1]}'. Must be a positive number.`);
  }
  ...
  if (!Number.isSafeInteger(bytes)) {
    throw new RepoxideError(`Invalid size: '${input}'. Resulting byte value is too large.`);
  }
