...
interface RepoxideButtonOptions {
  text: string;
  href: string;
  iconSrc?: string;
}
...
// Constants
const BUTTON_CLASS = 'repoxide-button';
const ICON_SIZE = 16;
const REPOXIDE_BASE_URL = 'https://repoxide.com';
const BUTTON_TEXT = 'Repoxide';
const DEFAULT_ICON_PATH = 'images/icon-64.png';

// Button functions
function isRepoxideButtonAlreadyExists(): boolean {
  return document.querySelector(`.${BUTTON_CLASS}`) !== null;
}

function createRepoxideButton(options: RepoxideButtonOptions): HTMLElement {
  ...
  button.title = chrome.i18n.getMessage('openWithRepoxide');
  ...
  octicon.innerHTML = `<img src="${iconSrc}" width="${ICON_SIZE}" height="${ICON_SIZE}" alt="Repoxide">`;
...
function addRepoxideButton(): void {
  try {
    // Check if button already exists
    if (isRepoxideButtonAlreadyExists()) {
      return;
    }
    ...
    // Create Repoxide URL
    const repoxideUrl = `${REPOXIDE_BASE_URL}/?repo=${encodeURIComponent(repoInfo.url)}`;

    // Create button
    const buttonContainer = createRepoxideButton({
      text: BUTTON_TEXT,
      href: repoxideUrl,
    });
    ...
    console.log(`Repoxide button added for ${repoInfo.owner}/${repoInfo.repo}`);
  } catch (error) {
    console.error('Error adding Repoxide button:', error);
  }
}
...
function observePageChanges(): void {
  ...
  window.addEventListener('popstate', () => {
    setTimeout(() => addRepoxideButton(), 100);
  });
}

function initRepoxideIntegration(): void {
  try {
    addRepoxideButton();
    observePageChanges();
  } catch (error) {
    console.error('Error initializing Repoxide integration:', error);
  }
}
...
export default defineContentScript({
  ...
  main() {
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => initRepoxideIntegration());
    } else {
      initRepoxideIntegration();
    }
  },
});
