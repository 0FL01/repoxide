...
import { cleanupTempDirectory, copyOutputToCurrentDirectory, createTempDirectory, resolveZipEntryPath } from './utils/fileUtils.js';
...
async function extractZipWithSecurity(file: File, destPath: string): Promise<void> {
  try {
    ...
    for (const entryPath of filePaths) {
      ...
      const normalizedPath = path.normalize(resolveZipEntryPath(destPath, entryPath));
    ...
    for (const [filePath, data] of Object.entries(files)) {
      ...
      const fullPath = resolveZipEntryPath(destPath, filePath);
