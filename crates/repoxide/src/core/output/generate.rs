...
pub fn generate_header(config: &OutputContextConfig) -> String {
    ...
    if processing_notes.is_empty() {
        format!(
            "{}, combined into a single document by Repoxide.",
            description
        )
    } else {
        format!(
...
pub fn generate_summary_notes(config: &OutputContextConfig) -> String {
    let mut notes = vec![
        "- Some files may have been excluded based on .gitignore rules and Repoxide's configuration".to_string(),
...
mod tests {
    ...
    fn test_generate_header_entire_codebase() {
        ...
        assert!(header.contains("Repoxide"));
    }
