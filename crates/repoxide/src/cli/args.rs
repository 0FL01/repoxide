...
//!
//! This module defines all command-line arguments and subcommands for repoxide.

use clap::{Parser, Subcommand, ValueEnum};
use std::path::PathBuf;

/// Repoxide - Pack your repository into a single AI-friendly file
#[derive(Parser, Debug, Clone)]
#[command(name = "repoxide")]
#[command(author = "Repoxide Contributors")]
#[command(version)]
#[command(about = "Pack repository contents to single file for AI consumption")]
#[command(
    long_about = "Repoxide is a tool that packs your entire repository into a single, \
...
pub struct Args {
    ...
    /// Use custom config file instead of repoxide.config.json
    #[arg(short, long, value_name = "PATH")]
    pub config: Option<PathBuf>,

    /// Create a new repoxide.config.json file with defaults
    #[arg(long)]
    pub init: bool,
...
pub enum Command {
    ...
    /// Initialize a repoxide.config.json file
    Init {
...
impl OutputStyle {
    ...
    pub fn default_file_name(&self) -> &'static str {
        match self {
            OutputStyle::Xml => "repoxide-output.xml",
            OutputStyle::Markdown => "repoxide-output.md",
            OutputStyle::Json => "repoxide-output.json",
            OutputStyle::Plain => "repoxide-output.txt",
        }
    }
...
mod tests {
    ...
    fn test_output_style_default_file_name() {
        assert_eq!(OutputStyle::Xml.default_file_name(), "repoxide-output.xml");
        assert_eq!(
            OutputStyle::Markdown.default_file_name(),
            "repoxide-output.md"
        );
        assert_eq!(
            OutputStyle::Json.default_file_name(),
            "repoxide-output.json"
        );
        assert_eq!(
            OutputStyle::Plain.default_file_name(),
            "repoxide-output.txt"
        );
    }
