...
use crate::config::{load_config, MergedConfig, RepoxideConfig};
...
fn run_init_action(cwd: &Path, global: bool) -> Result<()> {
    ...
    let config_path = config_dir.join("repoxide.config.json");
    ...
    let default_config = RepoxideConfig::default();
...
fn merge_cli_with_config(args: &Args, file_config: RepoxideConfig) -> MergedConfig {
...
mod tests {
    ...
    fn test_merge_cli_with_config_respects_file_style() {
        ...
        use crate::config::schema::RepoxideConfig;
        ...
        let mut file_config = RepoxideConfig::default();
        ...
        assert_eq!(merged.output.file_path, "repoxide-output.md");
    }
    ...
    fn test_merge_cli_with_config_overrides_file_style() {
        ...
        // Should auto-adjust file path
        assert_eq!(merged.output.file_path, "repoxide-output.json");
    }
}
