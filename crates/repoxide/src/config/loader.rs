...
//!
//! This module handles loading repoxide configuration from various file formats.

use anyhow::{Context, Result};
use std::fs;
use std::path::Path;

use super::schema::RepoxideConfig;
...
const GLOBAL_CONFIG_DIR: &str = "repoxide";
...
/// 3. Search for global config files in ~/.config/repoxide/
/// 4. Return default configuration if no config found
pub fn load_config(directory: &Path, config_path: Option<&Path>) -> Result<RepoxideConfig> {
...
fn find_global_config() -> Option<std::path::PathBuf> {
    ...
    // Also check home directory ~/.repoxide/
...
fn load_config_file(path: &Path) -> Result<RepoxideConfig> {
...
fn parse_json(content: &str, path: &Path) -> Result<RepoxideConfig> {
    serde_json::from_str(content)
        .with_context(|| format!("Failed to parse JSON config file: {:?}", path))
}

/// Parse JSONC (JSON with comments) or JSON5 config
fn parse_jsonc(content: &str, path: &Path) -> Result<RepoxideConfig> {
...
mod tests {
    ...
    fn test_load_default_config() {
        ...
        assert_eq!(config.output.file_path, "repoxide-output.xml");
    }

    #[test]
    fn test_load_json_config() {
        let dir = tempdir().unwrap();
        let config_path = dir.path().join("repoxide.config.json");
    ...
    fn test_load_jsonc_config() {
        let dir = tempdir().unwrap();
        let config_path = dir.path().join("repoxide.config.jsonc");
    ...
    fn test_load_config_with_trailing_commas() {
        let dir = tempdir().unwrap();
        let config_path = dir.path().join("repoxide.config.json5");
