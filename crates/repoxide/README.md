...
Высокопроизводительная реализация инструмента Repoxide на Rust. Предназначена для упаковки кодовой базы в единый файл (XML, Markdown, JSON, Plain) для контекста LLM.
...
cargo build -p repoxide --release

# Бинарный файл будет находиться здесь:
./target/release/repoxide
```
...
repoxide --style markdown --compress

# Указание выходного файла
repoxide -o codebase.md
```
...
repoxide --include "src/**/*.rs,Cargo.toml"

# Игнорирование (дополнительно к .gitignore)
repoxide --ignore "tests/**,benches/**"
```
...
```bash
# GitHub shorthand
repoxide --remote yamadashy/repoxide

# Полный URL с веткой
repoxide --remote https://github.com/user/repo.git --branch develop
```
...
Поддерживается `repoxide.config.json` (стандартный JSON или JSONC с комментариями).
