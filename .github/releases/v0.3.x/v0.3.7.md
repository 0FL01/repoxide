...
We've added a powerful `grep_repoxide_output` tool to the Repoxide MCP server! This feature enables AI assistants to perform advanced searches within packaged codebases. Additionally, the `read_repoxide_output` tool now supports partial content retrieval.
...
By combining the `grep_repoxide_output` and `read_repoxide_output` tools, repository analysis is now possible without fetching the entire output, making it particularly useful for analyzing medium to large-scale repositories.

For MCP setup instructions, see:
https://github.com/yamadashy/repoxide?tab=readme-ov-file#mcp-server-integration
...
```bash
$ repoxide -h
Usage: repoxide [options] [directories...]
