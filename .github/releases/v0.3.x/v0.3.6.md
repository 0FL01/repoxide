...
- `--no-file-summary` now also suppresses the generation header
  ...
  - Users can now control headers more flexibly and intuitively, showing custom headers while hiding Repoxide-generated content

Special thanks to @joshwand for this significant improvement!

### JSON Schema Configuration Validation (#570)
- Added JSON schema for `repoxide.config.json` validation
...
- Added Docker container support for running Repoxide as an MCP server
```json
{
  "mcpServers": {
    "repoxide-docker": {
