...
Resolved a critical issue where the `--stdin` option would hang when piping file paths to Repoxide.

The problem occurred because v1.6.0 moved the entire CLI processing to child_process workers for better resource isolation, but child_process workers don't inherit stdin from the parent process by default. This caused commands like `git ls-files | repoxide --stdin` to hang indefinitely.
