This release fixes an issue with output file extensions introduced in v1.9.0 and reduces package size, making Repoxide more intuitive and efficient!
...
# Output: repoxide-output.xml (wrong extension!)
```

**After (v1.9.1):**
```bash
repoxide --style markdown
# Output: repoxide-output.md ✨
```
...
Replaced `tree-sitter-wasms` with `@repomix/tree-sitter-wasms`, which contains only the language parsers needed for Repoxide. This reduces the total package size by approximately **25%**, resulting in faster installation times and lower disk usage.
