This release introduces TypeScript/JavaScript config support and Azure DevOps integration, making Repoxide more flexible and compatible with enterprise development environments!
...
```typescript
// repoxide.config.ts
import { defineConfig } from 'repoxide';

const today = new Date().toISOString().slice(0, 10);

export default defineConfig({
  output: {
    filePath: `repoxide-output-${today}.xml`,
    style: 'xml',
  },
});
```
...
**Priority Order:**
1. **TypeScript** (`repoxide.config.ts`, `repoxide.config.mts`, `repoxide.config.cts`)
2. **JavaScript/ES Module** (`repoxide.config.js`, `repoxide.config.mjs`, `repoxide.config.cjs`)
3. **JSON** (`repoxide.config.json5`, `repoxide.config.jsonc`, `repoxide.config.json`)

### Azure DevOps Remote Repository Support (#848, #881)
Added support for Azure DevOps remote repository URLs, making Repoxide compatible with enterprise environments using Azure DevOps.

Azure DevOps uses special URL formats:
- SSH: `git@ssh.dev.azure.com:v3/org/project/repo`
- HTTPS: `https://dev.azure.com/organization/project/_git/repo`

Repoxide now correctly parses and processes these URLs when packing remote repositories.
