This release introduces git commit history integration and enhanced binary file detection, making Repoxide more informative for AI analysis and user visibility!
...
repoxide --include-logs

# Include specific number of commits
repoxide --include-logs --include-logs-count 10
...
Repoxide now reports files detected as binary by content inspection, providing better visibility into excluded files.
...
The Repoxide website now supports URL query string parameters, allowing users to:
- Bookmark configurations with preset options
- Share links with predefined settings

Example URLs with preset configurations:
- https://repoxide.com/?format=markdown&include=**%2F*.ts&ignore=**%2F*.test.ts
- https://repoxide.com/?repo=yamadashy%2Frepoxide&format=markdown&include=**%2F*.ts&ignore=**%2F*.test.ts
