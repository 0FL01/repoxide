...
We've also submitted Repoxide to the MCP marketplace:
- https://github.com/cline/mcp-marketplace/issues/64

To use Repoxide as an MCP server with Cline (VS Code extension), edit the `cline_mcp_settings.json` file:
```json
{
  "mcpServers": {
    "repoxide": {
...
https://github.com/yamadashy/repoxide#mcp-integration
...
**Known Issue**: There's currently an issue where negation patterns (`!`) don't work correctly. See [Issue #400](https://github.com/yamadashy/repoxide/issues/400) for details.
