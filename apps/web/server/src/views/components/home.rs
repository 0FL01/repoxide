    ...
    html! {
        div class="theme-switcher" data-theme-switcher {
            div class="theme-toggle" role="group" aria-label=(t(locale, "Color theme", "Цветовая тема")) {
                (render_theme_toggle_button("system", "🖥️", t(locale, "System", "Системная"), true))
                (render_theme_toggle_button("light", "☀️", t(locale, "Light", "Светлая"), false))
...
fn render_theme_toggle_button(
    ...
    html! {
        button
            ...
            title=(label)
        {
            span class="theme-toggle__emoji" aria-hidden="true" { (emoji) }
        }
    }
}
